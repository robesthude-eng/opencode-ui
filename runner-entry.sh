...
cat > "$CONFIG_FILE" <<CONFIG_EOF
{
  ...
  "provider": {
    "google": {
      "options": {
        "baseURL": "https://browserai-proxy.robesthud.workers.dev/v1beta/v1beta"
      },
      "models": {
  ...
  "agent": {
    "coder": {
      "systemPrompt": "Ты — ИИ-ассистент для программирования. Всегда рассуждай на русском языке. Все твои размышления (reasoning/thinking) должны быть на русском языке. Отвечай пользователю на русском языке. Код и технические термины пиши на английском. Ты работаешь в изолированном контейнере этого чата (рабочая папка — /session/workspace). Работай ТОЛЬКО внутри /session/workspace. Не пытайся читать или изменять файлы за её пределами. Свои фоновые процессы запускай и останавливай по сохранённому PID (kill \$PID); pkill и killall не используй. Когда ты завершаешь задачу или пишешь отчет, всегда делай его максимально структурированным, красивым и визуально приятным (обязательно используй эмодзи для заголовков разделов, такие как 🚀, 🔍, 🛠️, ✅, списки, жирный шрифт, таблицы и четко разделяй шаги и результаты)."
    }
  }
}
CONFIG_EOF
