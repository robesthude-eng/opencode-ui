...
function buildServer() {
  ...
  server.registerTool(
    ...
    async ({ page_id }) =>
      runTool(async () => textResult(await notion.getPage(page_id))),
  );
  ...
  server.registerTool(
    ...
    async ({ block_id }) =>
      runTool(async () => textResult(await notion.getBlockChildren(block_id))),
  );
    ...
    "create_page",
    {
      ...
      inputSchema: {
        ...
        markdown: z
          .string()
          .optional()
          .describe("Markdown content to convert into blocks."),
    ...
    "append_blocks",
    {
      ...
      inputSchema: {
        ...
        after_block_id: z
          ...
          .optional()
          .describe(
            "Optional id of an existing direct child block; new blocks are inserted immediately after it. Must be a direct child of page_id.",
          ),
      },
    },
    async (args) =>
    ...
    "delete_blocks",
    {
      ...
      inputSchema: {
        ...
        block_ids: z
          .array(z.string())
          .min(1)
          .describe("Block ids or block URLs to remove from the page."),
      },
    },
    async (args) =>
      runTool(async () =>
  ...
  server.registerTool(
    "sync_markdown_file",
    {
      ...
      inputSchema: {
        file_path: z
          .string()
          .describe("Absolute or relative Markdown file path."),
        page_id: z.string().optional().describe("Existing page id to replace."),
        parent_page_id: z
          .string()
          .optional()
          .describe("Parent page id when creating a new page."),
    ...
    async (args) =>
      runTool(async () => {
        ...
        if (!args.parent_page_id) {
          throw new Error(
            "`parent_page_id` is required when `page_id` is not provided.",
          );
        }

        const title =
          args.title || inferTitleFromMarkdown(file.content, file.absolutePath);
