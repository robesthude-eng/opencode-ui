	...
	return (
		<div className="rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-4 space-y-3">
			<div className="flex items-start justify-between gap-3">
				<div>
					<h4 className="font-semibold text-sm flex items-center gap-2">⚡ Мгновенный откат UI</h4>
					<p className="text-xs text-muted-foreground mt-1">
						Возвращает последнюю удачную сборку без npm/vite (обычно &lt;1 с). Используйте,
						если после саморазвития UI сломался.
					</p>
				</div>
				<Button
					size="sm"
					className="shrink-0 bg-emerald-600 hover:bg-emerald-500 text-white"
					disabled={!!instantStatus || !selfImproveEnabled || distSnapshots.length < 2}
							...
							>
								...
								{!s.current && selfImproveEnabled && (
									<Button
										...
										onClick={() => {
											const currentIndex = distSnapshots.findIndex((item) => item.current);
											handleInstantRollback(
