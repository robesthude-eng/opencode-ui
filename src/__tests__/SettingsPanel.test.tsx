...
describe("SettingsPanel", () => {
  ...
  test("switches between tabs", () => {
    ...
    fireEvent.click(within(nav).getByText("О системе"));
    expect(screen.getByText("OpenCode UI (Cloud Edition)")).toBeInTheDocument();
    fireEvent.click(within(nav).getByText("Модели"));
    expect(screen.getByText("Свой API-ключ (BYOK)")).toBeInTheDocument();
  });

  test("filters sections with settings search", () => {
    render(<SettingsPanel />);
    const nav = desktopNav();
    const search = within(nav).getByLabelText("Поиск по настройкам");
    fireEvent.change(search, { target: { value: "ключ" } });
    expect(within(nav).getByText("Модели")).toBeInTheDocument();
    expect(within(nav).queryByText("О системе")).not.toBeInTheDocument();
  });

  test("hides admin section for non-admin users", () => {
    setState({ currentUser: { role: "user", email: "user@example.com" } });
    render(<SettingsPanel />);
