...
- Name: full-ui.spec.ts >> 6. Workspace panel >> 6.5 Upload folder button visible
- Location: e2e/full-ui.spec.ts:446:3
...
Locator: locator('button:has-text("Upload folder")').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('button:has-text("Upload folder")').first()
...
> 448 |     await expect(uploadBtn).toBeVisible();
  ...
  532 |       // Should see provider list
  533 |     }
  534 |   });
  535 | 
  536 |   test("7.7 Switch to About tab", async ({ page }) => {
  537 |     const tab = page
  538 |       .locator(
  539 |         'button[role="tab"]:has-text("About"), button:has-text("О приложении")',
  540 |       )
  541 |       .first();
  542 |     if (await tab.isVisible({ timeout: 1000 }).catch(() => false)) {
  543 |       await tab.click();
  544 |       await page.waitForTimeout(500);
  545 |       // Should see version info
  546 |       await expect(
  547 |         page.locator("text=/0\\.3\\.1|version|OpenCode/i").first(),
  548 |       ).toBeVisible({
```