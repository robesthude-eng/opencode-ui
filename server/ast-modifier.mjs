    ...
    readBody(req, MAX_JSON_BODY_BYTES).then((buf) => {
      try {
        ...
        if (!filePath || !Array.isArray(operations) || operations.length === 0) {
          ...
          res.end(JSON.stringify({ error: "Missing or invalid 'filePath' or 'operations' array in payload." }));
          return;
        }
        ...
        try {
          ...
          if (diagnostics.length > 0) {
            ...
            throw new Error(`Modified code has syntax errors: ${firstErr.messageText} at character ${firstErr.start}`);
          }

          // Format code using Prettier (if available)
          import("./sandbox.mjs").then((sb) => {
            ...
            import("child_process").then(({ execFile }) => {
              execFile("npx", ["prettier", "--write", tmpFilePath], { timeout: 10000 }, (prettierErr) => {
                ...
                if (dryRun) {
                  ...
                  res.end(JSON.stringify({
                    status: "success",
                    message: "AST modification check succeeded! Code is clean and syntactically correct.",
                    content: content
                  }));
                } else {
                  // Write back to the active repository
                  logAudit(WORKDIR, userEmail, "AST_MODIFY_DEPLOY_START", `Modifying ${filePath} via AST`);
                  fs.writeFileSync(resolvedPath, content, "utf8");
                  
                  // Auto Git Checkpoint
                  createGitCheckpoint(activeUiDir, filePath, (gitErr, commitMessage) => {
                    if (gitErr) {
                      logAudit(WORKDIR, userEmail, "AST_MODIFY_DEPLOY_WARNING", `AST changes applied to ${filePath} but Git commit failed: ${gitErr.message}`);
                      res.writeHead(200, { "Content-Type": "application/json" });
                      res.end(JSON.stringify({ status: "success", message: "AST changes deployed successfully, but Git checkpoint failed." }));
                    } else {
                      logAudit(WORKDIR, userEmail, "AST_MODIFY_DEPLOY_SUCCESS", `AST changes deployed to ${filePath}. Commit: ${commitMessage}`);
                      res.writeHead(200, { "Content-Type": "application/json" });
                      res.end(JSON.stringify({ status: "success", message: "AST changes deployed successfully!", commit: commitMessage }));
                    }
                  });
                }
              });
          ...
          res.writeHead(400, { "Content-Type": "application/json" });
          res.end(JSON.stringify({ status: "error", error: "AST operation failed", detail: opErr.message }));
        }
...
function addImport(sourceText, moduleSpecifier, namedImports = [], defaultImport = null) {
  ...
  if (existingImport) {
    const currentNamed = [];
    if (existingImport.importClause && existingImport.importClause.namedBindings && ts.isNamedImports(existingImport.importClause.namedBindings)) {
    ...
    if (toAdd.length === 0 && (!defaultImport || (existingImport.importClause && existingImport.importClause.name && existingImport.importClause.name.text === defaultImport))) {
      return sourceText;
    }
    ...
    if (defaultImport || (existingImport.importClause && existingImport.importClause.name)) {
...
function addRoute(sourceText, method, routePath, handlerCode) {
  ...
  const routeTrigger = `urlPath === "${routePath}"`;
  ...
  if (index === -1) {
    throw new Error("Could not find the hook location '// Self-improvement endpoints — ADMIN ONLY.' in server code.");
  }
