...
function saveUsersObject(data) {
  const tx = sqlite.transaction(() => {
    const existing = sqlite
      ...
      .all()
      .map((r) => r.email);
    const incomingEmails = Object.keys(data || {}).map((e) => e.toLowerCase());
    const deleted = existing.filter((e) => !incomingEmails.includes(e));

    if (deleted.length > 0) {
      const delStmt = sqlite.prepare("DELETE FROM users WHERE email = ?");
      for (const email of deleted) delStmt.run(email);
    }

    const ins = sqlite.prepare(
      "INSERT OR REPLACE INTO users (email, password_hash, role, created_at) VALUES (?, ?, ?, ?)",
    );
...
function saveSessionsObject(data) {
  const tx = sqlite.transaction(() => {
    const existing = sqlite
      ...
      .all()
      .map((r) => r.token);
    const incomingTokens = Object.keys(data || {});
    const deleted = existing.filter((t) => !incomingTokens.includes(t));

    if (deleted.length > 0) {
      const delStmt = sqlite.prepare("DELETE FROM sessions WHERE token = ?");
      for (const token of deleted) delStmt.run(token);
    }
...
function saveOwnersObject(data) {
  const tx = sqlite.transaction(() => {
    const existing = sqlite
      .prepare("SELECT session_id FROM session_owners")
      .all()
      .map((r) => r.session_id);
    const incomingSids = Object.keys(data || {});
    ...
    const ins = sqlite.prepare(
      "INSERT OR REPLACE INTO session_owners (session_id, email) VALUES (?, ?)",
    );
